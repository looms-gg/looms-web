...
export function ResetPasswordPage() {
  ...
  const [status, setStatus] = useState<Status>(() => (hasSession ? "ready" : "wait"))
  ...
  return (
    <div className="mx-auto max-w-md py-16 text-center">
      {status === "wait" ? <ResetPasswordWait /> : null}
      {status === "missing" ? <ResetPasswordMissing /> : null}
      {status === "done" ? <ResetPasswordDone /> : null}
      {status === "ready" ? <ResetPasswordForm onDone={() => setStatus("done")} /> : null}
    </div>
  )
}
