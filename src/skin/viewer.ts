...
import { FULL_GROUPS, PART_GROUP, SKIN_PARTS, type SkinPart } from "./compose"
...
/**
 * Regions a render should cover: declared covers win; otherwise a full figure
 * covers every rack and a single-group crop stays on its rack.
 */
function coveredGroups(group: Group | "full", covers?: Group[]): readonly Group[] {
  if (covers?.length) return covers
  return group === "full" ? FULL_GROUPS : [group]
}
...
export function visibleSkinParts(
  ...
  const parts = coveredGroups(group, covers)
