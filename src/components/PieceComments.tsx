      ...
      {editing ? (
        <CommentComposer
          ...
          onSubmit={async (body) => {
            setError(null)
            if (!viewerId) return
            const { error: err } = await updateGarmentComment(comment.id, viewerId, body)
            if (err) {
      ...
      <div className="flex flex-wrap gap-1">
        ...
        {canDelete ? (
          <button
            ...
            onClick={() => {
              setError(null)
              void deleteGarmentComment(comment.id).then(({ error: err }) => {
                if (err) {
                  setError(formatErrorMessage(err))
                  return
                }
      ...
      {replyOpen ? (
        <CommentComposer
          ...
          onSubmit={async (body) => {
            setError(null)
            if (!viewerId) return
            const { error: err } = await createGarmentComment({
